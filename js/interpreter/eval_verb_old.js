...
var js_verbs = {
	...
	,   "tailrec"   : "[] linrec"
	,   "binrec"    : ""
	,   "genrec"    : function (stack, env) {
            var [cond, t, r1, r2] = stack.pops(4, [["array"], ["array"], ["array"], ["array"]]);
            var tmp_stack = j_dup(stack);
            var cond_res = j_eval(cond, tmp_stack, env).pops(1, [["boolean"]]);

            if (cond_res) {
                j_eval(t, stack, env)
            } else {
                j_eval(r1, stack, env);
                j_eval([[cond], [t], [r1], [r2], Symbol.for("genrec")], stack, env);
                j_eval(r2, stack, env);
            }
        }
	,   "condlinrec": ""
